...
def window2():
    ...
    line_1=canv.create_line(330,250,260,150,width=3)
    px, py, ox, oy = 300,250,random.randint(210,290),150
    if 230>ox:
        error=str(int(230-ox))+"-"
    else:
        error=str(int(ox-230))+"+"
    line_2 = canv.create_line(px, py, ox, oy, width=4, fill="grey")
    root.title("Эксперимент")
    canv.focus_set()
    canv.pack()
    root.after(120000,lambda x=1:root.destroy())
